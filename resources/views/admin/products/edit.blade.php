...
@section('content')
    ...
    <form class="admin-one admin-form"
          action="{{route('user.admin.products.update', $product->id)}}"
        ...
        <div class="admin-row">
            ...
            <div class="admin-input form-input">
                <div class="dropdown" id="types_dropdown">
                    @error("type_id")
                    <div class="required_alert">{{$message}}</div>
                    @enderror
                    <label for="category">Виберіть тип :</label>
                    <select name="type_id" id="type" class="select" class="{{$errors->has('category') ? 'required' : '' }}">
                        @foreach($types as $type)
                            <option value="{{$type->id}}"
                                {{$product->type->id == $type->id ? "selected" : ""}}>
                                {{$type->value}}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="admin-row">
            ...
            <div class="admin-input form-input">
                <label for="discount">Ціна товара: </label>
                    ...
                    <select class="select multi-select" multiple id="sizes" name="sizes[]">
                        @foreach($sizes as $size)
                            <option value="{{ $size->id }}"
                                {{ $product->hasSize($size->id) ? "selected" : "" }}>
                                {{ $size->value }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="admin-row">
            <div class="admin-input">
                ...
                <div class="file-input">
                    ...
                    <input type="file" id="file_input" class="file-input__input" name="image[]" multiple accept="image/png, image/gif, image/jpeg" />
...
@push('js')
    <script>
        let fileImages = @json($product->images ? $product->images : '');
        let fileNames = '';
        for (const file of fileImages) {
            fileNames += `<span>${file.filename}</span>`;
        }
